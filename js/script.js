...
const postItColors = ["#ffff88", "#ff9999", "#99ff99", "#99ccff", "#ffcc99", "#ff99ff"];
const dateOptions = { weekday: 'short', day: '2-digit', month: '2-digit', year: '2-digit' };
...
const formatDate = (date) => new Date(date).toLocaleDateString('id-ID', dateOptions);
...
const makeTodo = ({ id, task, priority, deadline, isCompleted, timestamp }) => {
  ...
  container.className = `item shadow ${isCompleted ? "completed" : ""} ${overdue ? "overdue" : ""}`;
  container.style.backgroundColor = getPostItColor(id);

  const createdDate = formatDate(timestamp || new Date());
  const deadlineText = deadline ? formatDate(deadline) : "Tidak ada";
  ...
  container.innerHTML = `
    ...
    <div class="checkbox-container">
      <input type="checkbox" class="neo-checkbox" ${isCompleted ? "checked" : ""}>
    </div>
  `;

  container.querySelector(".neo-checkbox").onchange = (e) => toggleTodoStatus(id, e.target.checked);
  return container;
};
...
const updateCurrentTime = () => {
  ...
  if (timeElement) {
    timeElement.innerHTML = `
      ...
      <p>${now.toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" })}</p>
      <p>${now.toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit", second: "2-digit" })}</p>
    `;
  }
};
...
document.addEventListener(RENDER_EVENT, () => {
  ...
  activeTodos.sort((a, b) => {
    ...
    return aOverdue !== bOverdue ? bOverdue - aOverdue : priorityOrder[b.priority] - priorityOrder[a.priority];
  });
  ...
  document.getElementById("delete-all-btn").style.display = completedTodos.length > 0 ? "block" : "none";
  updateStatistics();
});
